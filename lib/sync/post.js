...
var Post = function(doc, type){
  ...
  if(doc.type && doc.ts) {
    ...
    var isSubtypeFileShare   = (subtype == 'file_share');
    var isSubtypeFileComment = (subtype == 'file_comment');
    
    var userId = isSubtypeFileComment ? doc.comment.user : doc.user;
    var slackUser = userList[userId];
    if (!slackUser) {
    ...
    if (isSubtypeFileShare) {
      this.message = isFileImage ? '[圖片]' : '[附件]';
    ...
    } else if (isSubtypeFileComment) {
      this.message = (isFileImage ? '[圖片回覆]' : '[附件回覆]') + '\n' + doc.comment.comment;
    } else {
      this.message = formatter.doc.text;
    }
    ...
    // note that non-image attachments are dropped
    if ((isSubtypeFileShare || isSubtypeFileComment) && isFileImage) {
    ...
    // finally, apply formatter
    this.message = formatter.formatSlackMessage(this.message);
