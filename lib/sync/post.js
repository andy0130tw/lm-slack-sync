...
var LM_SINGLE_MESSAGE_LENGTH_LIMIT = 256;
...
var Post = function(doc, type){
  ...
  if(doc.type && doc.ts) {
    ...
    if (doc.type == 'post') {
      ...
      if (doc.initial_comment) {
        this.message + ' ' + doc.initial_comment.comment;
      }
    } else if (doc.type =='file_comment') {
      this.message = '[圖片回覆]' + doc.comment.comment;
    } else {
